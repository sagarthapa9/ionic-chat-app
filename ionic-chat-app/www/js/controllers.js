...
/* Logicfor monitoring incoming chats.
We make a connection with the chats base URL and keep listening for changes on it.
If there is a new chat added or chat content is changed,
we will identify whether the new chat message is for the current user.
If it is, we broadcast a newChatHistory event that we will use in the history tab, to show new chats*/
     ...
     $ionicPlatform.ready(function () {
         ...
         $scope.$on('showChatInterface', function ($event, authData) {
             ...
             $scope.onlineusers.$loaded().then(function () {
                 $scope.onlineusers.$add({ picture: authData.cachedUserProfile.picture, name: authData.displayName, email: authData.email, login: Date.now() })
                     .then(function (ref) {
...
// Get a list of users who are online and display them
...
.controller('ChatDetailCtrl', ['$scope', 'Loader', '$ionicPlatform', '$stateParams', 'UserFactory', 'FBFactory', '$ionicScrollDelegate',
    ...
    function ($scope, Loader, $ionicPlatform, $stateParams, UserFactory, FBFactory, $ionicScrollDelegate, $cordovaImagePicker,
        ...
        $ionicPlatform.ready(function () {
            ...
            /*
            We use the $ionicScrollDelegate service to scroll the viewing pane to the bottom/last message. 
            Method that will add a new chat message  to Firebase
            */
            function postMessage(msg, type, map) {
                var d = new Date();
                d = d.toLocaleTimeString().replace(/:\d+ /, ' ');
                map = map || null; $scope.messages.$add({
                    content: msg,
                    time: d,
                    type: type,
                ...
                $scope.chatMsg = ''; $ionicScrollDelegate.scrollBottom(true);
            }

            //User enters a text and clicks on Send, we call the sendMessage method
            $scope.sendMessage = function () {
                if (!$scope.chatMsg)
                    return;
                var msg = '<p>' + $scope.user.cachedUserProfile.name + ' says : <br/>' + $scope.chatMsg + '</p>';
                var type = 'text'; postMessage(msg, type);
            }

            // SHOW ACTION SHEET TO SHARE TAKE PHOTO OR SHOW USER LOCATION
            ...
            $scope.centerOnMe = function () {
                ...
                $cordovaGeolocation.getCurrentPosition(posOptions).then(function (pos) {
                    $scope.user.pos = { lat: pos.coords.latitude, lon: pos.coords.longitude };
            ...
            $scope.selectLocation = function () {
                ...
                var map = { lat: pos.lat, lon: pos.lon };
                var type = 'geo';
                postMessage('<p>' + $scope.user.cachedUserProfile.name + ' shared : <br/>', type, map); $scope.modal.hide();
            }
...
Can implement preferences such as Show notification when the app is open, Play sound etc.
 */
.controller('AccountCtrl', ['$scope', 'FBFactory', 'UserFactory', '$state', function ($scope, FBFactory, UserFactory, $state) {
    $scope.logout = function () {
        ...
        } UserFactory.cleanPresenceId(); $state.go('main');
    }
}]);
